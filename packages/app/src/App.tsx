...
import CounterContext, {CounterContextType} from "../store/counter-context.tsx";
...
function App () {
  ...
  const [letterDensity, setLetterDensity] = useState<Map<string, number>>(new Map());
  ...
  function updateCounter (text: string, excludeSpaces: boolean) {
    ...
    const count = (s: string) => {
      return [...s].reduce((obj: Map<string, number>, char: string): Map<string, number> => {
        if (excludeSpaces){
